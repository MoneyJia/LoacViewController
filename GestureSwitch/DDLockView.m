...
- (void)setup
{
    for (int index = 0; index<9; index++) {
        // 创建按钮
        DDCircleView *btn = [DDCircleView buttonWithType:UIButtonTypeCustom];
        btn.tag = index;
        // 添加按钮
        [self addSubview:btn];
    }
...
- (void)layoutSubviews
{
    ...
    for (int index = 0; index<self.subviews.count; index++) {
        // 取出按钮
        DDCircleView *btn = self.subviews[index];
...
/**
 根据触摸点位置获得对应的按钮
 */
- (DDCircleView *)buttonWithPoint:(CGPoint)point
{
    for (DDCircleView *btn in self.subviews) {
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    // 2.获得触摸的按钮
    DDCircleView *btn = [self buttonWithPoint:pos];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([self.delegate respondsToSelector:@selector(lockView:didFinishPath:)]) {
        ...
        for (DDCircleView *btn in self.selectedButtons) {
...
- (void)drawRect:(CGRect)rect
{
    ...
    for (int index = 0; index<self.selectedButtons.count; index++) {
        DDCircleView *btn = self.selectedButtons[index];
